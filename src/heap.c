...
#define COMPARE(a, b) ((heap->compare(heap->items[a], heap->items[b]) > 0) ? (a) : (b))
...
void heap_insert(heap *heap, int data) {
    ...
    for (size_t child = heap->size - 1, parent = PARENT(child), min = COMPARE(parent, child);
         child > 0 && min != parent;
         child = parent, parent = PARENT(child), min = COMPARE(parent, child)) {
...
#define COMPARE3(a, b, c) (COMPARE((a), COMPARE((b), (c))))
